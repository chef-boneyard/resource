...
ChefDSL is an attempt to make Chef resources easier and more fun to write and use, as well as more powerful.  The 0.1 version aims to:
...
One bit of friction that you have to overcome to create a resource: if you are only using a resource in one place, you need to go off and create a recipe file.  ChefDSL eliminates this by letting you create resource types *anywhere*.  `libraries`, `resources` or even in a recipe!
...
ChefDSL.resource :bank_account do
...
The purpose of resources is to make it so that users don't have to type the same thing over and over.  Just about everyone who has created files and directories will sympathize with this: mode, owner and group need to be retyped each time.  ChefDSL introduces a new `defaults` mechanism to allow people to make specialized versions of a resource (or even renames) that have those values as their initial value.

### The ChefDSL Way

```ruby
ChefDSL.defaults :superuser_file, :file, mode: 0700, owner: 'root', group: 'wheel'
...
Full-on resources are super useful, and allow you to create reusable things that can both read and write; but sometimes you don't need quite so much.  Sometimes you're really just writing a recipe with nice properties.  In these cases, ChefDSL merges the best of the `definition` and `resource` worlds to give you the simplest resource possible, removing the need even for the word `recipe`:

### The ChefDSL Way

```ruby
ChefDSL.define :apache, config_dir: '/etc/httpd' do
  package 'apache2' do
  end
...
ChefDSL makes patchiness natural and easy with a new rule:
...
Implementing this is easy in ChefDSL: you add a `load` recipe:
...
By pulling `load_current_resource` into the resource and setting the default value of the resource's properties to their current value, ChefDSL has in effect created both a write *and* a read API.  You don't even have to modify anything.
...
It can sometimes be useful to say "I want this resource to be like this other one, but with a few extra things."  ChefDSL resources are built to be inheritance, from the declaration syntax on down.  Type `resource :name, Superclass do ... end` to declare one.
...
Because of Chef's two-phase "compile the recipes, then run them" model, lazy values are a key component of using Chef resources.  Both existing resources and ChefDSL resources and ChefDSL resources support them.  ChefDSL introduces some more compact syntax, as well as a conceptual optimization for lazy defaults.
...
Because properties can have types in ChefDSL, that means properties can have Resource types as well.  This concept is deeply embedded and resource references are easy to construct and use.  Witness:
...
ChefDSL is designed to minimize the number of steps a user has to take before they can write a resource.  Many starting Chef users know resources and recipes, but do not know Ruby.  ChefDSL makes the leap simple by treating the resource "action" as a `recipe` (and by using that terminology consistently to define it).

This means everything you know about recipes transfers: you can make resources, and they execute entirely the same way.

LWRPs already allow this; ChefDSL just moves it up a notch and reduces the friction between you and your resource.
...
ChefDSL offers a way to keep the power of lazy loading and load / converge these properties separately: `load_value` and `converge`.  To wit, here's a sample `file` resource:
...
ChefDSL defines automatic coercion from strings for `Integer`, `Boolean`, and `Symbol`, and automatic coercion to `String` with `to_s`, but you can define your own coercion as well.  We'll talk about that more in the coercion section.
...
ChefDSL doesn't change at all except to remove the .to_f:
...
In ChefDSL, we implement this by specializing the ChefDSL Type for Float and adding our own coercion:
...
#### Alternative ChefDSL: Doing It Inline
...
In ChefDSL, `do` on an `property` or `resource` puts you into the `class` definition for the thing in question.  In this case, the main way (defining Currency) makes more sense.
...
Because we had added this to the previous example, we didn't have to change much here.  If you were doing coercion as your first thing, you'd have to learn everything from the Strong Types example, but nothing more.  For ChefDSL, type coercion is where you have to learn the new thing.
...
In ChefDSL, we modify our currency type:
...
ChefDSL supports resource references as a first-class concept as well!  Just declare the property to have the Resource as its type.
...
ChefDSL freezes values (makes them readonly) once they have been defined, to allow for the copying and precalculation of values where desired.  It freezes identity values after the resource is declared, so that notifications cannot be thrown off; it freezes *all* values after the do block.
...
ChefDSL formalizes the concept of **identity** of a resource:
...
Many resources have multiple bits of data that make up their true identity.  Witness, for example, Chef's data bag item: its unique identification consists of both the data bag name, and the data bag item name.  ChefDSL allows for this.
...
In ChefDSL, you can mark identity properties as optional with default values:

```ruby
ChefDSL.resource :google_place do
...
In a number of cases, one resource is actually a part of the identity of another resource.  When that happens, ChefDSL strives to make it as painless as possible to construct and use the resource.
...
One issue with the previous example is that with all properties optional, you always have to specify name/value pairs to construct the resource; even if you can figure it out.  ChefDSL's resource system has a very easy way around this: redefine the `open` method on the resource!  The default `open` method (on the base class) is the one that decides the default way of constructing the resource.

### The ChefDSL Way

In ChefDSL, you replace the `coerce` method:
