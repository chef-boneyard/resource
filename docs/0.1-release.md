...
ChefResource is an attempt to make Chef resources easier and more fun to write and use, as well as more powerful.  The 0.1 version aims to:
...
One bit of friction that you have to overcome to create a resource: if you are only using a resource in one place, you need to go off and create a recipe file.  ChefResource eliminates this by letting you create resource types *anywhere*.  `libraries`, `resources` or even in a recipe!
...
### The ChefResource Way

```ruby
#
# bank/recipes/default.rb
ChefResource.resource :bank_account do
  property :account_id, String, index: true
...
The purpose of resources is to make it so that users don't have to type the same thing over and over.  Just about everyone who has created files and directories will sympathize with this: mode, owner and group need to be retyped each time.  ChefResource introduces a new `defaults` mechanism to allow people to make specialized versions of a resource (or even renames) that have those values as their initial value.

### The ChefResource Way

```ruby
ChefResource.defaults :superuser_file, :file, mode: 0700, owner: 'root', group: 'wheel'
...
# bank/resources/account.rb
  ...
  property :account_id, index: true
  property :full_name
  property :balance
  recipe do
...
Full-on resources are super useful, and allow you to create reusable things that can both read and write; but sometimes you don't need quite so much.  Sometimes you're really just writing a recipe with nice properties.  In these cases, ChefResource merges the best of the `definition` and `resource` worlds to give you the simplest resource possible, removing the need even for the word `recipe`:

### The ChefResource Way

```ruby
ChefResource.define :apache, config_dir: '/etc/httpd' do
  package 'apache2' do
  end
...
In general, Chef resources are *patchy*.  This means that if you specify only some properties, the other properties don't get blanked out: they stay the same (or in some cases get set to a default).  A canonical example: if you say this:
...
ChefResource makes patchiness natural and easy with a new rule:

> The default value of a property is its *current value* (if you don't say anything about a property, you don't want it to change).

Implementing this is easy in ChefResource: you add a `load` recipe:
...
By pulling `load_current_resource` into the resource and setting the default value of the resource's properties to their current value, ChefResource has in effect created both a write *and* a read API.  You don't even have to modify anything.

All you have to do to use it is call the resource, and don't specify `do`.  We will create the resource and let you read from it (but not write), and it won't be added to the resource collection so you don't have to worry about it trying to update or mucking up your pretty green text.

Here's some code that uses the resource we just built:
...
It can sometimes be useful to say "I want this resource to be like this other one, but with a few extra things."  ChefResource resources are built to be inheritance, from the declaration syntax on down.  Type `resource :name, Superclass do ... end` to declare one.

### The ChefResource Way

```ruby
resource :chef_object do
  property :chef_server_url, default: "https://api.opscode.com"
  property :organization, String
  property :name, String, identity: true
  property :raw_data, String, default: lazy { calc_raw_data }
  def calc_raw_data
...
Because of Chef's two-phase "compile the recipes, then run them" model, lazy values are a key component of using Chef resources.  Both existing resources and ChefResource resources and ChefResource resources support them.  ChefResource introduces some more compact syntax, as well as a conceptual optimization for lazy defaults.
...
```ruby
ChefResource.resource :budget do
  property :account_id, String, identity: true
  property :salary, Float
  property :spending_money, Float do
    default { salary / 12 }
  end
end
```
...
TODO make depends "resource" cause resources to use the new DSL and "resource" / "define" / "defaults" keyword
TODO make sure defaults :x, :y creates a resource so you can do defaults :z, :x
TODO if we are calculating an explicit lazy value for a property, and it asks for the value of that property, either *error* or get the current/default value?  That would allow spending_money { spending_money*0.5 } to have a useful meaning.
...
TODO solve issue of being able to type property :name, Rubygems (right now, the recipe doesn't have that class in scope and there is no way to change constant lookup scope for the class-definition block)
TODO related to above: allow user to type property :name, rubygems and have it work.  NOTE: this might not work at all if there is both a property and a type named rubygems--which type do we return?
...
Composition (resources as properties)
-------------------------------------

Because properties can have types in ChefResource, that means properties can have Resource types as well.  This concept is deeply embedded and resource references are easy to construct and use.  Witness:

### The ChefResource Way

```ruby
ChefResource.resource :budget do
  property :account, BankAccount, identity: true
  property :joint_account, BankAccount
  property :salary, Float
  property :spending_money, Float do
    default { salary / 12 }
  end
...
- Make properties with the type of other resources
...
ChefResource is designed to minimize the number of steps a user has to take before they can write a resource.  Many starting Chef users know resources and recipes, but do not know Ruby.  ChefResource makes the leap simple by treating the resource "action" as a `recipe` (and by using that terminology consistently to define it).

This means everything you know about recipes transfers: you can make resources, and they execute entirely the same way.

LWRPs already allow this; ChefResource just moves it up a notch and reduces the friction between you and your resource.
...
There are some features that are designed to make things easier down the road, too.  Certain resources are *compound resources*--that is, they have multiple properties, and those properties correspond to different actual things (some of which may take a long time to access).  `file` is one of those--`content` and `mode` are read and written in completely different ways, and `content` can be expensive to access, in particular (reading an entire file).

ChefResource offers a way to keep the power of lazy loading and load / converge these properties separately: `load_value` and `converge`.  To wit, here's a sample `file` resource:
...
resource :file do
  ...
  property :mode, Fixnum
  property :content, String, load_value: lazy { IO.read(file) }
  def load
...
To do this, we assign our properties strong types.  
...
ChefResource defines automatic coercion from strings for `Integer`, `Boolean`, and `Symbol`, and automatic coercion to `String` with `to_s`, but you can define your own coercion as well.  We'll talk about that more in the coercion section.
...
ChefResource doesn't change at all except to remove the .to_f:
...
In ChefResource, we implement this by specializing the ChefResource Type for Float and adding our own coercion:
...
#### Alternative ChefResource: Doing It Inline
...
In ChefResource, `do` on an `property` or `resource` puts you into the `class` definition for the thing in question.  In this case, the main way (defining Currency) makes more sense.
...
Because we had added this to the previous example, we didn't have to change much here.  If you were doing coercion as your first thing, you'd have to learn everything from the Strong Types example, but nothing more.  For ChefResource, type coercion is where you have to learn the new thing.
...
In ChefResource, we modify our currency type:
...
ChefResource supports resource references as a first-class concept as well!  Just declare the property to have the Resource as its type.
...
ChefResource freezes values (makes them readonly) once they have been defined, to allow for the copying and precalculation of values where desired.  It freezes identity values after the resource is declared, so that notifications cannot be thrown off; it freezes *all* values after the do block.
...
TODO might want to only ever freeze all or none, due to optional identity properties that users might want to *set* during the `do` block.  Have to think about this more.
...
Specifying `Path` as the type of your property restricts it to path strings (and will call `to_s` on Pathnames).  The type can have `relative_to`, which will allow users to specify relative paths (which are then appended to `relative_to`)

### The ChefResource Way

```ruby
ChefResource.resource :blah do
  property :url, Path, relative_to: '/foo', default: 'bar.txt'
end
blah 'hi' do
...
Specifying `Pathname` as the type of your property restricts it to Pathnames (and will parse Strings).  The type can have `relative_to`, which will allow users to specify relative paths (which are then appended to `relative_to`)

### The ChefResource Way

```ruby
ChefResource.resource :blah do
  property :url, Pathname, relative_to: '/foo', default: 'bar.txt'
end
blah 'hi' do
  puts path #=> /foo/bar.txt
  path 'blah.html'
...
Specifying `URI` as the type of your property restricts it to URIs (and will parse strings as URI).  The type can have `relative_to`, which will allow users to specify relative paths (which are then appended to `relative_to`)

### The ChefResource Way

```ruby
ChefResource.resource :blah do
  property :url, URI, relative_to: 'https://chef.io', default: 'index.html'
end
blah 'hi' do
...
ChefResource formalizes the concept of **identity** of a resource:
...
resource :bank_account do
  property :account_id, String, identity: true
  property :linked_account, BankAccount
  property :full_name, String
  property :balance, Float
end
...
Many resources have multiple bits of data that make up their true identity.  Witness, for example, Chef's data bag item: its unique identification consists of both the data bag name, and the data bag item name.  ChefResource allows for this.

### The ChefResource Way

```ruby
resource :data_bag do
  property :data_bag_name, String, identity: true
  property :data_bag_item_name, String, identity: true
end
...
In ChefResource, you can mark identity properties as optional with default values:

```ruby
ChefResource.resource :google_place do
  property :place_id,   Integer, identity: true, default: nil
  property :latitude,   Float,   identity: true, default: nil
  property :longitude,  Float,   identity: true, default: nil
  property :address,    Address, identity: true
...
resource :chef_server do
  property :url, Uri, identity: true, default: "https://api.opscode.com"
  property :organizations, load_value: lazy { HTTP.get("#{url}/organizations") }
end
...
In a number of cases, one resource is actually a part of the identity of another resource.  When that happens, ChefResource strives to make it as painless as possible to construct and use the resource.
...
```ruby
resource :bank do
  property :bank_name, String, identity: true
  property :routing_number, String
end

resource :bank_account do
  property :bank, Bank, identity: true
  property :account_id, Integer, identity: true
...
One issue with the previous example is that with all properties optional, you always have to specify name/value pairs to construct the resource; even if you can figure it out.  ChefResource's resource system has a very easy way around this: redefine the `open` method on the resource!  The default `open` method (on the base class) is the one that decides the default way of constructing the resource.

### The ChefResource Way

In ChefResource, you replace the `coerce` method:
...
The algorithm for deciding how the identity properties will look is:

1. Any required (non-defaulted) identity properties may be passed as positional arguments to the constructor:
```ruby
resource :foo do
  property :a, identity: true
  property :b, identity: true
  property :c, identity: true
  property :child, Foo
end
foo 1, 2, 3 do
  puts a # 1
...
```
2. *All* identity properties, whether they are identity or not, can be specified with keyword arguments:
```ruby
resource :foo do
  property :a, identity: true
  property :b, identity: true
  property :c, identity: true
  property :d, identity: true, default: 'ddd'
  property :e, identity: true, default: 'eee'
  property :f, identity: true, default: 'fff'
  property :child, Foo
end
...
3. If required identity properties are not passed or are passed both positionally and as keyword arguments, an error is thrown.
  ...
  - Nested resources
    - Resource types can have *contained* (child) resource types
      ...
      resource :bank do
        ...
        property :description, String
        resource :account do
          property :id, Integer, identity: true
          property :balance, Fixnum
        end
      end
      ```
