...
module ChefDSL
  module ChefDSL
    ...
    module ResourceDefinitionDSL
      ...
      def resource(name, base_resource_class=nil, class_name: nil, overwrite_resource: false, &override_block)
        ...
        resource_class = Chef::Resource.class_eval <<-EOM, __FILE__, __LINE__+1
          class #{class_name} < base_resource_class
            if !(self <= ChefDSL::ChefDSL::ChefResourceExtensions)
              include ChefDSL::ChefDSL::ChefResourceExtensions
              extend ChefDSL::ChefDSL::ChefResourceClassExtensions
            end
            self
          end
        EOM
      ...
      #
      # ChefDSL.define :resource_name, a: 1, b: 2 do
      #   file 'x.txt' do
      #     content 'hi'
      #   end
      # end
