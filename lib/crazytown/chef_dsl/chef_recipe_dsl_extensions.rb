...
module Crazytown
  module ChefDSL
    module ChefRecipeDSLExtensions
      #
      # Allow Crazytown resources to be declared with a different syntax:
      #
      ...
      def declare_resource(type, *identity, created_at, &update_block)
        ...
        if !resource_class.is_a?(Crazytown::Resource)
          case identity.size
          when 0
        ...
        # Crazytown resources!
        resource = build_crazytown_resource(resource_class, *identity, created_at, &update_block)
      ...
      def build_resource(type, *identity, created_at, &update_block)
        ...
        # Crazytown!
        build_crazytown_resource(resource_class, *identity, created_at, &update_block)
      end

      def build_crazytown_resource(resource_class, *identity, created_at, &update_block)
